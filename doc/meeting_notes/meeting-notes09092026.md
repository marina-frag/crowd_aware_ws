...
2. Υπάρχοντα vanilla DWAL με simulated odometry και Nav2 costmap σε κόσμο με στατικά εμπόδια και ανθρώπους, ώστε να εξετάζουμε τα paths/clusters του.
...
```mermaid
flowchart TD
    T["Έτοιμο keyboard teleop"] -->|"/cmd_vel: επιθυμητά v, ω"| S["Simulator: i-WALK, στατικά εμπόδια, crowd"]
    S -->|"/scan"| C["Nav2 local costmap"]
    S -->|"/odom και TF"| G["dwal_generator"]
    C -->|"OccupancyGrid"| G
    F["Footprint του i-WALK"] --> G
    G -->|"sampled_paths"| K["dwal_clustering"]
    G -->|"Path markers"| R["RViz"]
    K -->|"Cluster markers"| R
```
Το Nav2 obstacle layer μετατρέπει τις μετρήσεις του lidar σε costmap.
...
## Ερωτήσεις
1. Να βάλω τον shared controller και γιατι σταματά αφού δεν τον έχω ενσωματώσεις ακόμη?




## Coffee shop simulation notes
